...
from .decoder import ModulatedCustomMSDeformableAttention
...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformableAttention3D) or isinstance(m, TemporalSelfAttention) \
                    or isinstance(m, CustomMSDeformableAttention) or isinstance(m, ModulatedCustomMSDeformableAttention):
                try:
                    m.init_weight()
    ...
    def forward(self,
        ...
        object_query_dim=object_query_embed.size(1)
        query_pos, query = torch.split(
            object_query_embed, [self.embed_dims,object_query_dim-self.embed_dims], dim=1)
