...
class DaMapHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(DaMapHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.loss_pts = build_loss(loss_pts)
        self.loss_dir = build_loss(loss_dir)
        self.loss_cls_raw = build_loss(loss_cls_raw)
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_class = self.cls_branches[lvl](hs[lvl][:,:,:self.embed_dims]
                                            .view(bs,num_vec, self.num_pts_per_vec,-1)
                                            .mean(2))
            tmp = self.reg_branches[lvl](hs[lvl][:,:,self.embed_dims:])
    ...
    def loss_single(self,
        ...
        score_quality = label_weights.new_zeros(labels.shape)
        ...
        if len(pos_inds) > 0:
            ...
            # import pdb;pdb.set_trace()
            loss_pts_single = self.loss_pts(
                pts_preds[isnotnan,:,:][pos_inds].detach(), normalized_pts_targets[isnotnan,
                                                                :,:][pos_inds], 
                pts_weights[isnotnan,:,:][pos_inds],
                reduction_override='none'
                )
            score_quality[pos_inds]=torch.exp(-(loss_pts_single.mean(1).sum(-1))).detach()
        ...
        loss_cls = self.loss_cls(
            cls_scores, (labels,score_quality), label_weights, avg_factor=cls_avg_factor)
    ...
    def loss(self,
        ...
        # import pdb;pdb.set_trace()
        losses_cls_0, losses_bbox_0, losses_iou_0, losses_pts_0, losses_dir_0 = self.loss_single_raw(all_cls_scores[0], all_bbox_preds[0],all_pts_preds[0],all_gt_bboxes_list[0], all_gt_labels_list[0],all_gt_shifts_pts_list[0],all_gt_bboxes_ignore_list[0])
        
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            self.loss_single, all_cls_scores[1:], all_bbox_preds[1:],all_pts_preds[1:],
            all_gt_bboxes_list[1:], all_gt_labels_list[1:],all_gt_shifts_pts_list[1:],
        ...
        num_dec_layer = 0
        loss_dict[f'd{num_dec_layer}.loss_cls'] = losses_cls_0
        loss_dict[f'd{num_dec_layer}.loss_bbox'] = losses_bbox_0
        loss_dict[f'd{num_dec_layer}.loss_iou'] = losses_iou_0
        loss_dict[f'd{num_dec_layer}.loss_pts'] = losses_pts_0
        loss_dict[f'd{num_dec_layer}.loss_dir'] = losses_dir_0
